...
def postit(request):
	...
	return render_to_response('postits.html', { 'postits': postits }, content_type='text/html', context_instance=RequestContext(request))
...
def postit_edit_or_new(request, id=None):
	...
	return render_to_response('form.html', { 'form': form }, content_type='text/html', context_instance=RequestContext(request))
...
def postit_by_filter(request, filter=None, selector=None):
	...
	return render_to_response('postits_filtered.html', { 'postits': postits, 'status':selector }, content_type='text/html', context_instance=RequestContext(request))

def postit_export(request):
    data = serializers.serialize("xml", PostIt.objects.all())
    return render_to_response('export.xml', { 'data': data }, content_type='text/html', context_instance=RequestContext(request))
