...
from .forms import PostSearchForm, UpdatePictureForm, CreatePictureForm, TextForm, CreateURLForm, ProjectForms, TextUpdateForm
...
class ProjectIndex(LoginRequiredMixin, TemplateView):
	...
	def get(self, request, *args, **kwargs):
		...
		context['tag'] = URLData.objects.filter(folder__proj__user = self.request.user).order_by('-publish_date')[:4]
		context['folder'] = that
...
class ProjectCreateView(LoginRequiredMixin,FormView):
	...
	def post(self, request, *args, **kwargs):
		...
		elif 'cre_proj.x' in request.POST:
			context={}
			form= ProjectForms(request.POST)
			getProject = Project.objects.filter(name = request.POST['name']).exists()
			getemail = str(self.request.user.email)
			print(getemail)
			media_dir = settings.MEDIA_ROOT    
			new_dir_path = os.path.join(media_dir, getemail,request.POST['name'] )
			my_file = Path(new_dir_path)
			print(my_file)
			if my_file.is_file():
				pass
			else:
				os.makedirs(new_dir_path) #folder created 
			if not getProject:
				print(request)
				if form.is_valid():
					form.save()
...
#	fields = ['title', 'proj']
	...
	def post(self, request, *args, **kwargs):
		getFolder1 = Folder.objects.filter(proj__name= self.kwargs['slug'])
		if 'title' in request.POST:
			form = FolderForms(request.POST)	
			getFolder = Folder.objects.filter(title = request.POST['title']).exists()
			print(getFolder)
			getemail = str(self.request.user.email)
			getproj = request.POST.get('test')
			new_dir_path = os.path.join(settings.MEDIA_ROOT, getemail,getproj,request.POST['title'] )
			my_file = Path(new_dir_path)
			if not getFolder:
				if form.is_valid():
					form.save()
				...
				context['name'] = Folder.objects.filter(proj__name= self.kwargs['slug'])
				context['project'] = Project.objects.get(name= self.kwargs['slug'])
				return render(request,"moodboard/Folder_create.html", context)
...
class PictureCreateView(FormView):
	...
	def post(self,request, *args, **kwargs):
		...
		context['Pro_view'] = Project.objects.filter(name=self.kwargs['slug']) & Project.objects.filter(user__id = self.kwargs['pk'])
		context['Folder_view'] = Folder.objects.filter(title=self.kwargs['ti']) & Folder.objects.filter(proj__name=self.kwargs['slug']) & Folder.objects.filter(proj__user__id = self.kwargs['pk'])
...
class Delete(DeleteView):
	...
	def get_success_url(self):
		return reverse_lazy('Project:create_pic',kwargs ={'pk': self.object.folder.proj.user.id,'slug':self.object.folder.proj.name,'ti':self.object.folder.title})

class URLDelete(DeleteView):
	model = URLData

	def get_success_url(self):
		return reverse_lazy('Project:create_url',kwargs ={'pk': self.object.folder.proj.user.id,'slug':self.object.folder.proj.name,'ti':self.object.folder.title})

def index(request, slug, ti):
	model = TextData
	form = TextForm()
	text = TextData.objects.filter(folder__title = ti)

	context = {'form': form, 'Pro_view': Project.objects.filter(name=slug) & Project.objects.filter(user__id = request.user.id), 'Folder_view': Folder.objects.filter(title=ti) & Folder.objects.filter(proj__name=slug) & Folder.objects.filter(proj__user__id = request.user.id), 'tag': URLData.objects.filter(folder__proj__user = request.user).order_by('-publish_date')[:4]}



	if text:
		context['text'] = text.order_by('-id')[0].text
	else:
		context['text'] = text
	...
	if request.method == 'POST':
		...
		if not text:
			if 'text_save' in request.POST: 
				
				form = TextForm(request.POST)
				if form.is_valid():
					print(12345)
					form.save()
					text = TextData.objects.filter(folder__title = ti).order_by('-id')[0].text
					context['text'] = text
			...
			if 'text_save' in request.POST:
				text = TextData.objects.filter(folder__title = ti).order_by('-id')[0]
				
				form = TextUpdateForm(request.POST, instance = text)
				if form.is_valid():
					form.save(id = text.pk)
					context['text'] = TextData.objects.filter(folder__title = ti).order_by('-id')[0].text
...
class URLCreateView(FormView):
	...
	def get_context_data(self, **kwargs):
		...
		context['Pro_view'] = Project.objects.filter(name=self.kwargs['slug']) & Project.objects.filter(user__id = self.kwargs['pk'])
		context['Folder_aut'] = Folder.objects.filter(proj__name=self.kwargs['slug']) & Folder.objects.filter(proj__user__id = self.kwargs['pk']) & Folder.objects.filter(title=self.kwargs['ti'])
		context['Folder_all'] = Folder.objects.filter(proj__name=self.kwargs['slug']) & Folder.objects.filter(proj__user__id = self.kwargs['pk'])
